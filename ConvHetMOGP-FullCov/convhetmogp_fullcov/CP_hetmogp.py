# Code of Correlated Chained GPs with Convoltion processes was develped by Juan-Jose Giraldo
# based on the previous codes of the HetMOGP model by Pablo Moreno-Munoz (Universidad Carlos III)
# University of Sheffield

import numpy as np
import GPy
from convhetmogp_fullcov.util import draw_mini_slices
from convhetmogp_fullcov.svmogp_inf import SVMOGPInf
...
class ConvHetMOGP(GPy.core.SparseGP):
    def __init__(self, X, Y, Z, kern_list_uq, kern_list_Gx, likelihood, Y_metadata, name='ConvHetMOGP', batch_size=None):
        ...
        super(ConvHetMOGP, self).__init__(X=Xmulti_batch[0][1:10], Y=Ymulti_batch[0][1:10], Z=Z, kernel=kern_list_uq[0], likelihood=likelihood,
        ...
        chols = choleskies.triang_to_flat(np.tile(3.0*np.eye(self.num_inducing)[None,:,:], (self.num_latent_funcs,1,1)))
        #V_ini = np.zeros((self.num_inducing, self.num_inducing))
        #for i in range(5):
        #    A = 0.2*np.random.randn(self.num_inducing, self.num_inducing)
        #    A2 = np.dot(A, A.T)
        #    V_ini = V_ini + A2
        #chols = choleskies.triang_to_flat(np.tile(V_ini[None, :, :], (self.num_latent_funcs, 1, 1)))
    ...
    def parameters_changed(self):
        ...
        for q, kern_q in enumerate(self.kern_list):
            ...
            for d in range(J):
                ...
                util.update_conv_Kfu(kern_q, self.kern_list_Gdj[d], self.kern_aux)
    ...
    def posteriors_F(self, Xnew, which_out=None):
        ...
        for d in range(D):
            if f_index[d]==indix_aux[d]:
                ...
                q_fd, _ = self.inference_method.calculate_q_f(X=Xtask, Z=self.Z, q_U=q_U,S_u=S_u,p_U=p_U, kern_list=self.kern_list,kern_list_Gdj=self.kern_list_Gdj, kern_aux=self.kern_aux, B=self.B_list,
                ...
                posterior_fd = Posterior(mean=q_fd.m_fd.copy(), cov=q_fd.S_fd.copy(),
                                         K=util.conv_function_covariance(X=Xtask, B=self.B_list, kernel_list=self.kern_list, kernel_list_Gdj=self.kern_list_Gdj, kff_aux=self.kern_aux, d=d),
