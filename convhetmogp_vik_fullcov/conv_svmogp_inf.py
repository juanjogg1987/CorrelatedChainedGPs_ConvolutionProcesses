...
#qu = recordtype("q_U", "mu_u chols_u")
pu = namedtuple("p_U", "Kuu Luu Kuui")

class SVMOGPInf(LatentFunctionInference):

    def inference(self, q_u_means, q_u_chols, X, Y, Z, kern_list, kern_list_Gdj,kern_list_Tq, kern_aux,likelihood, B_list, Y_metadata, KL_scale=1.0,
        ...
        Kuu, Luu, Kuui = util.VIK_covariance(Z, kern_list,kern_list_Tq,kern_aux)
        ...
        for d in range(D):
            Xtask = X[f_index[d]]
            q_fd, q_U = self.calculate_q_f(X=Xtask, Z=Z, q_U=q_U,S_u=S_u ,p_U=p_U, kern_list=kern_list, kern_list_Gdj=kern_list_Gdj,kern_list_Tq=kern_list_Tq, kern_aux=kern_aux, B=B_list,
        ...
        # inference for rest of cases
            ...
            #if np.isnan(VE_dm).sum()>0 or np.isnan(VE_dv).sum()>0:
            #    print('nan')
            for t in range(T):
    ...
    def calculate_q_f(self, X, Z, q_U,S_u, p_U, kern_list, kern_list_Gdj,kern_list_Tq, kern_aux, B, M, N, Q, D, d):
        """
        ...
        # Algebra for p(f_d|u):
        Kfdu = util.conv_cross_covariance_VIK(X, Z, B, kern_list, kern_list_Gdj, kern_list_Tq, kern_aux, d)
